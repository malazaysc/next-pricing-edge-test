...
export function calculatePrice(data: InterfacePriceData): number {
  let totalPrice = 0;
  ...
  const totalDays =
    Math.round(
      Math.abs(
        (Date.parse(convertDate(startDate)) -
          Date.parse(convertDate(endDate))) /
          oneDay
      )
    ) + 1;
      ...
      applicableRules.sort((a, b) => {
        ...
        if (a.date && b.date) {
          if (a.fixedPrice !== undefined && b.fixedPrice !== undefined) {
            ...
            a.priceModifier !== undefined &&
            b.priceModifier !== undefined
          ) {
